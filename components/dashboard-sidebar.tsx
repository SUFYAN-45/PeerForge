...
import {
  Shield, Activity, Building2, Heart, Home,
  LayoutDashboard, Users, Package, AlertTriangle,
  Settings, ChevronLeft, ChevronRight, Zap, Lock,
  MapPin
} from "lucide-react"
...
const commandNav: NavItem[] = [
  ...
  { label: "Pulse Map",   href: "/command#map",        icon: MapPin, badge: "Live" },
  { label: "Smart Match", href: "/command#match",      icon: Users },
  { label: "Analytics",   href: "/command#analytics",  icon: Activity },
  { label: "Alerts",      href: "/command#alerts",     icon: AlertTriangle },
]
...
const portals = [
  { id: "frontline",  label: "Frontline",      href: "/frontline",  icon: Activity,    color: "text-emerald-400" },
  { id: "command",    label: "Command Center", href: "/command",    icon: Building2,   color: "text-cyan-400"    },
  { id: "benefactor", label: "Benefactor",     href: "/benefactor", icon: Heart,       color: "text-rose-400"    },
]

// ─── Mobile Bottom Nav ────────────────────────────────────────────────────────

function MobileBottomNav({ role, navItems }: { role: string; navItems: NavItem[] }) {
  const pathname = usePathname()

  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-sidebar/95 backdrop-blur-xl border-t border-sidebar-border px-2 pb-safe-area-inset-bottom">
...
export function DashboardSidebar({ role }: { role: "frontline" | "command" | "benefactor" }) {
  ...
  // Prefer publicMetadata (secure), fall back to unsafeMetadata (legacy)
  const userRole =
    (user?.publicMetadata?.role as string) ||
    (user?.unsafeMetadata?.role as string)

  const navItems =
    role === "frontline" ? frontlineNav :
    role === "command"   ? commandNav   :
    benefactorNav
    ...
    <>
      ...
      >
          ...
          <Link href="/" className="flex items-center gap-3">
            ...
            <AnimatePresence>
              {!isCollapsed && (
                ...
                >
                  <span className="font-semibold text-foreground truncate">RescueShield</span>
          ...
          {portals.map((portal) => {
            ...
            if (isAllowed) {
              return (
                ...
                >
                  <Icon className={cn("w-5 h-5 shrink-0", portal.color)} />
                  <AnimatePresence>
                    {!isCollapsed && (
                      <motion.span
                        initial={{ opacity: 0, x: -10 }}
            ...
            return (
              <div
                ...
                title={`Access restricted — ${portal.label} portal`}
              >
        ...
        <div className="p-3 border-t border-sidebar-border space-y-1">
          ...
          <div className={cn("flex items-center gap-3 px-3 py-1.5", !isCollapsed && "justify-between")}>
      ...
      {/* ── Mobile Bottom Nav ── */}
      <MobileBottomNav role={role} navItems={navItems} />
    </>
  )
}
